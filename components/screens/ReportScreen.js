...
export default function ReportScreen(props) {
    ...
    useEffect(() => {
        if (myChildren.length === 0 && !selectedChild.student_name) getMyChildren();
        else if (selectedChild.student_name) {
            getReport();
            getReportHistory();
        }
        ...
        if (!user.parent_id) {
            ...
            new Promise(
                ...
                new Promise(
                    ...
                    setSelectedChild({...res.data[0], school_id: res2.user.school_id, user_id: res2.user_id});
                }).catch(err => console.log(err))
                                        ...
                                        <TouchableWithoutFeedback
                                            onPress={async () => {
                                                ...
                                                config(options)
                                                    .fetch(
                                                        'GET',
                                                        `https://api.unison.id/reports/parent/${selectedChild.school_id}/${selectedChild.student_id}/${selectedChild.year_id}/${isParent ? user.parent_id : selectedChild.user_id}`,
                                                    )
                                                    .then((res) => {
                                                        if (Platform.OS === "ios") {
